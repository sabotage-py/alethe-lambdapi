...
let process (i : input) : (_, _) result =
  try
    let l = match i with File file -> S.parse_file_exn file in
...
let reconstruction file =
  ...
  match script with
  | Ok s ->
      T.get_lp_script Format.std_formatter
        s (*Format.printf "%a" pp_proofScript s*)
...
let process_files l =
  ...
  if List.length l = 2 then
    ...
    let res =
      match process_file f_smt2 with
