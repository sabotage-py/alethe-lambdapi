...
let smt2lp_stmt out (st:statement) = match view st with
  | Stmt_set_info (_,_) -> fpf out ""
  | Stmt_set_logic s -> let pp_set_logic = match s with 
    | "QF_UF" -> fpf out "require open lfroma.qfuf_encoding ; \n"
    | _ -> raise (UnderConstruction "The following logic is not yet supported.") in 
    pp_set_logic
  | Stmt_set_option _ -> fpf out ""
  | Stmt_decl_sort (s,n) -> let pp_decl_sort = match n with 
    | 0 -> fpf out "constant symbol %s : TYPE;" s
    | _ -> raise (UnderConstruction "Parameterized sort are not yet supported.") in 
    pp_decl_sort
  | Stmt_assert _ -> fpf out ""
  | Stmt_decl d -> if d.fun_ty_vars = [] then fpf out "symbol %s : %a ; \n" d.fun_name pp_ty d.fun_ret else 
      raise (UnderConstruction "I don't know what to do in this case")
  | Stmt_fun_def fr ->
